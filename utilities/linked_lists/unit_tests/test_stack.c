...
void test_stack_init_with_value(void)
{
    ...
    *actual_value = expected_value+32;
    ...
    TEST_ASSERT_EQUAL(&expected_value, s.top->obj);
    TEST_ASSERT_EQUAL(expected_value, *((int*)s.top->obj));
    TEST_ASSERT_EQUAL(expected_size, s.size);
    TEST_ASSERT(stack__pop(&s, (void**)&actual_value));
    TEST_ASSERT_EQUAL(NULL, s.top);
...
void test_series_of_random_values_push_and_pop(void)
{
    ...
    TEST_ASSERT_EQUAL(0, s.size);
    TEST_ASSERT_EQUAL(NULL, s.top);
    TEST_ASSERT_FALSE(stack__pop(&s, (void**)&actual_value));
}
